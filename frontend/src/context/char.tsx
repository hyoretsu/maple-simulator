...
export const CharProvider: React.FC<PropsWithChildren> = ({ children }) => {
	...
	useEffect(() => {
		const storedInfo = localStorage.getItem("@maple-simulator:char_info");
		if (storedInfo === null) {
			localStorage.setItem("@maple-simulator:char_info", JSON.stringify(charInfo));
	...
	const equips = useMemo<Record<string, Equipment>>(() => {
		...
		for (const equip of data as Equipment[]) {
			if (equip.type === "Pendant" || equip.type === "Ring") {
				for (const [key, value] of Object.entries(charInfo.equips).filter(([key]) =>
					key.match(/pendant|ring\d/),
				)) {
					if (value.id === equip.id) {
						Object.assign(equipsObject, { [key]: equip });
					}
				}
	...
	const updateInfo = useCallback(
		(info: string, value: any) => {
			...
			localStorage.setItem("@maple-simulator:char_info", JSON.stringify(updatedInfo));
		},
		[charInfo],
	);
